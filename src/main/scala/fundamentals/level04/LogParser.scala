...
object LogParser {
  ...
  final case class Error(severity: Int) extends LogLevel
  ...
  def parseLog(str: String): LogMessage = {
    ...
    val optLog: Option[LogMessage] = fields match {
      case List("I", timestampStr, message) =>
        parseIntOption(timestampStr).map(KnownLog(Info, _, message))
      case List("W", timestampStr, message) =>
        parseIntOption(timestampStr).map(KnownLog(Warning, _, message))
  ...
  def getErrorsOverSeverity(logs: List[LogMessage], severity: Int): List[LogMessage] = {
    logs.collect {
      case msg @ KnownLog(Error(errSeverity), _, _) if errSeverity > severity => msg
    }
  }
